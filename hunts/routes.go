...
func Routes(env *config.Env) *chi.Mux {
	...
	router.Post("/", createHuntHandler())              // tested
	...
	router.Get("/{huntID}/invitations/", getHuntInvitationsHandler())
	router.Delete("/{huntID}/invitations/{invitationID}", deleteHuntInvitationHandler())
	router.Post(
